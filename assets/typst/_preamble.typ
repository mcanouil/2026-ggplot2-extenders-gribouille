...
#import "assets/typst/theme-comic.typ": theme-comic, comic-title, comic-colours, comic-red, comic-blue, comic-yellow, comic-ink
...
// theme-comic keeps its comic-red accent default, so it can't reuse _wrap-theme
// (whose accent default is gold). Inherit ink/paper from the injected vars.
#let _theme_comic = theme-comic
#let theme-comic(ink: auto, paper: auto, accent: comic-red, ..fields) = {
  let args = (accent: accent)
  if ink != auto {
    args.insert("ink", ink)
  } else if _typst_render_foreground != none {
